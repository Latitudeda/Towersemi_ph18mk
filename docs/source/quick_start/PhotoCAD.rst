...
This section demonstrates how to import the Tower PH18MK PDK into **PhotoCAD**. It is divided into two steps: creating a PhotoCAD workspace and importing the Tower PH18MK PDK. If a PhotoCAD workspace has already been created, the user can skip to step 2.
...
Import Tower PH18MK PDK
**********************************************************

First, make sure that the Tower PH18MK PDK version matches the version of **PhotoCAD**. The version of **PhotoCAD** can be obtained in ``Run`` by running scripts in **PhotoCAD**.

Copy the ``towersemi_ph18mk`` folder to the workspace folder. As shown in the figure below, the ``towersemi_ph18mk`` folder is in the demo folder and parallel to .venv_demo.


.. image:: ../images/PhotoCAD_6.png

It's important to note that the PDK folder structure is the same as in the figure. Subfolders such as ``components`` and ``examples`` should be seen when expanding the ``towersemi_ph18mk`` folder. Otherwise, the import path will be wrong when you run the code.

Open and run ``towersemi_ph18mk/examples/tech_demo.py``. PDK will be imported successfully if there are no errors and ``tech_demo.gds`` will be exported.
